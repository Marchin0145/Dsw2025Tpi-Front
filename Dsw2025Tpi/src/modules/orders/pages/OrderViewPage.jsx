...
const formatDate = (dateString) => {
  if (!dateString || dateString.startsWith('0001-01-01')) return 'No especificada';
  ...
  <div className="flex flex-col gap-4 h-full p-4">
    <Card className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => nav('/admin/orders')} style="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <div className="space-y-2">
            <p><span className="font-medium">Cliente:</span> {order.nameCustomer}</p>
            <p><span className="font-medium">Fecha:</span> {parsearFechaAR(order.date)}</p>
            ...
            {order.orderItems.map((item, index) => (
              <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm text-gray-500">${item.currentUnitPrice} c/u</p>
                </div>
              </div>
            ))}
          </div>
