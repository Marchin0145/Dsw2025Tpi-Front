...
function RegisterPage() {
  ...
  const onValid = async (data) => {
    ...
    const { data: response, error } = isAdmin
    ...
    <div className="min-h-screen bg-teal-50 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white/90 backdrop-blur-sm p-8 shadow-xl rounded-xl border-0">
        <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">
          {isAdmin ? "Registro de Empleado" : "Registro de Usuario"}
        </h1>
        <form className="flex flex-col gap-4" onSubmit={handleSubmit(onValid)}>
          ...
          <Button
            weight={"w-full"}
            type="submit"
            style="bg-teal-500 hover:bg-teal-600 text-white border-2 border-teal-500 hover:border-teal-600 px-6 py-3 rounded-xl font-semibold transition-all duration-200 shadow-sm hover:shadow-md"
          >
            Registrarse
          </Button>
          <Button
            ...
            type="button"
            onClick={() => navigate(-1)}
            style="bg-gray-500 hover:bg-gray-600 text-white border-2 border-gray-500 hover:border-gray-600 px-6 py-3 rounded-xl font-semibold transition-all duration-200 shadow-sm hover:shadow-md mt-3"
          >
            Volver
          </Button>
        </form>
